 ...
 * 输入输出与副作用：只有严格 schema 和类型；WebUI 共享范围显式覆盖该管理入口的新会话，
 * 平台聊天仍须选择已有 canonical scope。契约版本不匹配由处理器明确拒绝。
 */
import { z } from "zod";

export const MEMORY_INGESTION_VERSION = 1;
...
export const WEB_MEMORY_SCOPE_ID = "kaguya:memory:webui";
...
export const memoryIngestionJobsSchema = z
  ...
  .strict();
export const memoryIngestionOptionsSchema = z
  .object({
    scopes: z.array(
      z.object({ informationId: id, label: z.string() }).strict(),
    ),
  })
  .strict();
