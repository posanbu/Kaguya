...
export class InformationCore implements OneShotScheduleCorePort {
  ...
  async scheduleOneShot(input: OneShotScheduleRequest): Promise<OneShotScheduleReceipt> {
    const schedules = this.store.oneShotSchedules;
    if (!schedules) throw new Error("One-shot scheduling is not configured");
    const atom = await this.buildOneShotAtom(oneShotRequestedInformationKind, {
      operationKey: input.operationKey, dueAt: input.dueAt, input: input.input, activation: { ...input.activation },
    }, [{ relation: "core:caused-by", informationId: input.sourceInformationId }, ...(input.references ?? [])]);
    const guard = this.currentScheduleGuard();
    const result = await schedules.create({ operationKey: input.operationKey, schedule: atom, dueAt: input.dueAt, ...(guard ? { guard } : {}) });
    return result;
  }

  async replaceOneShot(input: OneShotScheduleReplacement): Promise<OneShotReplacementReceipt> {
    const schedules = this.store.oneShotSchedules;
    if (!schedules) throw new Error("One-shot scheduling is not configured");
    const atom = await this.buildOneShotAtom(oneShotRequestedInformationKind, {
      operationKey: input.operationKey, dueAt: input.dueAt, input: input.input, activation: { ...input.activation },
    }, [{ relation: "core:caused-by", informationId: input.sourceInformationId }, { relation: "core:replaces", informationId: input.previousScheduleInformationId }, ...(input.references ?? [])]);
    const superseded = await this.buildOneShotAtom(oneShotSupersededInformationKind, {}, [{ relation: "core:status-of", informationId: input.previousScheduleInformationId }]);
    const guard = this.currentScheduleGuard();
    return schedules.replace({ operationKey: input.operationKey, previousScheduleInformationId: input.previousScheduleInformationId, schedule: atom, superseded, dueAt: input.dueAt, ...(guard ? { guard } : {}) });
  }

  async finishOneShot(input: OneShotTerminalRequest): Promise<OneShotTerminalResult> {
    const schedules = this.store.oneShotSchedules;
    if (!schedules) throw new Error("One-shot scheduling is not configured");
    const definition = input.status === "fired" ? oneShotFiredInformationKind : oneShotFailedInformationKind;
    const payload = input.status === "fired" ? {} : { failureKind: input.failureKind };
    const atom = await this.buildOneShotAtom(definition, payload, [{ relation: "core:status-of", informationId: input.scheduleInformationId }]);
    const guard = this.currentScheduleGuard();
    return schedules.finish({ scheduleInformationId: input.scheduleInformationId, terminal: atom, ...(guard ? { guard } : {}) });
  }

  private currentScheduleGuard(): OneShotFencingGuard | undefined {
    const execution = this.#execution.getStore();
    return execution ? { ...execution.claim, signal: execution.signal } : undefined;
  }
