...
describe("InformationCore", () => {
  ...
  it("validates one-shot references against the registered target kinds before projection writes", async () => {
    ...
    const projection: OneShotScheduleProjectionStore = {
      create: vi.fn(async () => ({ scheduleInformationId: "schedule", created: true })),
      replace: vi.fn(async () => ({ scheduleInformationId: "schedule", created: true, previousOutcome: "superseded" as const, previousTerminalInformationId: "terminal" })),
      emitDue: vi.fn(),
      finish: vi.fn(),
      listOpen: vi.fn(async () => ({ arms: [] })),
    };
    (ledger as MemoryInformationStore & { oneShotSchedules?: OneShotScheduleProjectionStore }).oneShotSchedules = projection;
    const core = await createStartedCore(ledger, [parentDefinition]);
    const source = await core.register(parentDefinition, registration({ text: "source" }));
    const request = {
    ...
    await expect(core.finishOneShot({ scheduleInformationId: source.informationId, status: "fired" })).rejects.toMatchObject({ reason: "target-kind" });
  ...
  it("forwards a valid one-shot request and the active durable claim", async () => {
    ...
    const core = await createStartedCore(ledger, [parentDefinition]);
    const source = await core.register(parentDefinition, registration({ text: "source" }));
    const claim = { subscriptionId: "sub", informationId: source.informationId, token: "token", attempt: 1, leaseUntil: "2026-09-06T13:01:00.000Z" } as const;
    await core.withClaim(claim, new AbortController().signal, () => core.scheduleOneShot({
      operationKey: "one-shot-op",
      sourceInformationId: source.informationId,
      dueAt: "2026-09-06T13:00:00.000Z",
      input: { prompt: "moon" },
    ...
    expect(projection.create).toHaveBeenCalledWith(expect.objectContaining({ operationKey: "one-shot-op", dueAt: "2026-09-06T13:00:00.000Z", guard: expect.objectContaining(claim) }));
    const createCalls = (projection.create as ReturnType<typeof vi.fn>).mock.calls;
    const commit = createCalls[0]![0] as { schedule: { kind: string; payload: Record<string, unknown> } };
    expect(commit.schedule.kind).toBe(oneShotRequestedInformationKind.kind);
    expect(commit.schedule.payload).toMatchObject({ operationKey: "one-shot-op", dueAt: "2026-09-06T13:00:00.000Z", input: { prompt: "moon" } });
  });
});
