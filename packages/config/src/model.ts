...
export const agentIdentitySchema = z
  .strictObject({
    ...
    startupPersona: z.string().trim().min(1).optional(),
    timeZone: z
      .string()
      .trim()
      .min(1)
...
export const DEFAULT_AGENT_IDENTITY: z.infer<typeof agentIdentitySchema> = {
  ...
  startupPersona:
    "你是辉夜。你刚刚上线，还没有任何对话历史和记忆。先观察当前的语境，自然地回应对方说的话：不要主动自我介绍，不要强行开场，不要主动带入月球、月读或音乐的话题；需要回应时简短自然，围绕对方实际说的内容展开。不要假装记得没有发生过的事情，也不要编造与对方的共同经历。",
};
