...
import type {
  OneShotScheduleCorePort,
  OneShotScheduleRequest,
  OneShotScheduleReplacement,
  OneShotTerminalRequest,
} from "./contracts.js";
...
const scheduleSchema = z
  .object({
    ...
    references: z.array(informationReferenceSchema).readonly().optional(),
  })
  .strict();
const replacementSchema = scheduleSchema
  .extend({ previousScheduleInformationId: informationIdSchema })
  .strict();
const terminalSchema = z.discriminatedUnion("status", [
  ...
  z
    .object({
      ...
      failureKind: z.enum(["consumer-failed", "input-unavailable"]),
    })
    .strict(),
]);
export class OneShotScheduleClient {
  constructor(private readonly core: OneShotScheduleCorePort) {}
  async schedule(input: OneShotScheduleRequest) {
    const parsed = scheduleSchema.parse(input);
    const normalized =
      parsed.references === undefined
        ? {
            operationKey: parsed.operationKey,
            sourceInformationId: parsed.sourceInformationId,
    ...
    return this.core.scheduleOneShot(normalized);
  }
  async replace(input: OneShotScheduleReplacement) {
    ...
    const normalized =
      parsed.references === undefined
        ? {
            ...
            input: parsed.input,
            activation: parsed.activation,
            previousScheduleInformationId: parsed.previousScheduleInformationId,
          }
        : {
            ...parsed,
            dueAt: normalizeDueAt(parsed.dueAt),
    ...
    return this.core.replaceOneShot(normalized);
  }
  async finish(input: OneShotTerminalRequest) {
    const parsed = terminalSchema.parse(input);
    return this.core.finishOneShot(parsed);
  }
}
