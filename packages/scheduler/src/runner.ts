...
import type {
  OneShotDueCommit,
  ScheduleClock,
  OneShotScheduleProjectionStore,
} from "./contracts.js";
...
export class DurableOneShotScheduler {
  ...
  readonly #timers = new Map<
    InformationId,
    { handle: unknown; generation: number; dueAt: string }
  >();
  ...
  constructor(options: DurableOneShotSchedulerOptions) {
    ...
    this.#batch = options.recoveryBatchSize ?? 256;
    this.#drain = options.drainTimeoutMs ?? 5000;
    if (!Number.isSafeInteger(this.#batch) || this.#batch < 1)
      throw new Error("Invalid scheduler recovery batch size");
    if (!Number.isSafeInteger(this.#drain) || this.#drain < 0)
      throw new Error("Invalid scheduler drain timeout");
  }
  start(): Promise<void> {
  ...
  async stop(): Promise<void> {
    ...
    const timeout = new Promise<void>((resolve) => {
      timeoutHandle = this.#clock.setTimeout(resolve, this.#drain);
    });
  ...
  async refresh(scheduleInformationId: InformationId): Promise<void> {
    ...
    while (true) {
      const page = await this.#store.listOpen({
        ...(cursor ? { after: cursor } : {}),
        limit: this.#batch,
      });
      if (
        ...
        return;
      const arm = page.arms.find(
        (candidate) =>
          candidate.scheduleInformationId === scheduleInformationId,
      );
      if (arm) {
        if (
          this.#state !== "started" ||
        ...
        if (Date.parse(arm.dueAt) <= this.#clock.now().getTime())
          await this.fire(
            ...
            this.#timers.get(arm.scheduleInformationId)?.generation ?? 0,
          );
        return;
      }
  ...
  private async fire(id: InformationId, generation: number): Promise<void> {
    ...
    const due = freezeInformationAtom({
      informationId: this.#nextInformationId(),
      kind: oneShotDueInformationKind.kind,
      occurredAt: this.#clock.now().toISOString(),
      source: "core:scheduler",
      payload: {
        scheduleInformationId: id,
        dueAt: timer.dueAt,
        deliveredAt: this.#clock.now().toISOString(),
      },
      references: [{ relation: "core:status-of", informationId: id }],
    });
    const work = this.#store
      .emitDue({ scheduleInformationId: id, due } as OneShotDueCommit)
      .then(
        () => {
