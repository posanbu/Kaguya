/**
 * 功能概述：显式模板资源的本地覆盖存储；内置文件与代码默认值始终只读。
 * 主要职责：readPromptResources 读取来源；writePromptOverride/removePromptOverride 只操作白名单 local 文件；
 * validatePromptResources 使用受限编译器验证整个模块模板组，错误不包含源码片段。
 * 代码库关系：加载器和管理端共用资源注册表；调用方持有共享配置锁并执行组 revision 比较。
 * 输入输出与副作用：读写未渲染模板，拒绝符号链接及异常文件，限制大小；临时文件 fsync 后 rename。
 */
import {
  constants,
  lstatSync,
...
const resources = new Map<
  string,
  {
    declaration: ModulePromptTemplateDefinition;
    fileStem: string;
    builtin?: string;
  }
>([
  ...messageModulePromptTemplates.map(
    (declaration) =>
      [
        declaration.templateId,
        { declaration, fileStem: declaration.templateId },
      ] as const,
  ),
  [
    plannerTemplateDeclaration.templateId,
    {
      declaration: plannerTemplateDeclaration,
      fileStem: "heartflow.planner",
      builtin: DEFAULT_PLANNER_TEMPLATE,
    },
  ],
  [
    personFactTemplateDeclaration.templateId,
    { declaration: personFactTemplateDeclaration, fileStem: "person-fact" },
  ],
]);
...
function file(root: URL, id: string, suffix: "local" | "default") {
  ...
  return new URL(`${resource(id).fileStem}.${suffix}.hbs`, root);
}
...
export function readPromptResources(
  ...
  return declarations.map((declaration) => {
    const entry = resource(declaration.templateId);
    const fallback =
      entry.builtin ?? read(file(root, declaration.templateId, "default"));
