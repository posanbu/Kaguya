...
An event stream starts with one top-level event carrying a `traceId`. It fans out through module nodes while preserving the same trace lineage, and may include route, reply, state, and memory model calls.
