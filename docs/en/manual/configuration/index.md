...
After replacing or selecting the active Profile, the UI enters
`restart_required` and asks the user to restart the Server so Runtime can
freeze the selected Profile again at startup.
...
Profile mutations through `/api/v1/profiles*`, and restart-required activation
for any selected-Profile change.
