...
用户手册先回答“在哪里改、有什么用、默认是什么、何时生效”。每页围绕一个任务，简短说明影响操作的限制。设计依据、数据流和内部契约放到开发文档，避免占据用户的安装与配置路径。
...
用户手册优先使用操作步骤、字段说明和可复制示例，不依赖 Mermaid 解释安装或配置。图表主要用于开发文档中难以用文字说明的关系；不要把同一流程同时写成图和长段落。
