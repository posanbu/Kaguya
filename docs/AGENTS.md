...
- `public/`：站点图标和静态资源。
- `design/`：当前界面设计与配置流程。
- `scripts/`：构建产物的站内链接、锚点和资源检查。

## 内容事实

文档必须以当前代码、根 README、`CONTRIBUTING.md` 和各 package README 为依据。历史资料已移至主分支之外的冻结归档，入口见 `README.md`；不得复制回文档源码或 `public/`。规划中的能力必须标为规划，不得写成已经可用。公开接口、事件、环境变量或启动方式变化时，同一提交更新对应页面。
