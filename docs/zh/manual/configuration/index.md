...
网关 token 由 Server 在启动时确定：未设置 `KAGUYA_GATEWAY_TOKEN` 时自动生成（日志 `server.token.generated`），Web UI 加载页面时自动获取，配置写入接口不会退化为匿名接口。匿名 setup 模式只能看到 metadata、issues 与 warnings；完整 Profile 的读取和修改仍要求管理认证。配置提交继续采用严格 schema，并在服务端校验 light/heavy 都引用已启用 Provider 中声明的模型；两个 tier 可以共享同一个目标。
