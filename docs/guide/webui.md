...
Profile 的“Agent 身份”区内嵌工作区名称、别名和 persona 资源编辑器。三者都不属于当前 Profile，修改会影响全部 Profile；Profile 本身只保存时区。保存与恢复默认都要求重启。平台参与策略仍在 Heartflow 模板页管理，平台表达风格在 Message Composer 模板页管理。
