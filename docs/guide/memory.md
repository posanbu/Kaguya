...
先选择适用范围和信息来源，再输入自然语言，例如“小夏喜欢天文，和小林是从小认识的朋友”。信息来源区分用户陈述和角色设定；人工输入不会自动成为已核实的事实。选择 **WebUI 的所有会话** 后，新建网页聊天也能按需召回；选择已有的平台群聊或私聊时，记忆只用于那个范围。

点击 **整理并保存** 后，系统会显示处理中、需要补充、已入库、部分完成或失败。出现同名人物时，选择对应身份并在输入区补充说明。同一次录入会保留上下文；点击 **新录入** 开始另一组内容。当前标签页中的 **录入记录** 可切回之前的会话；服务端记录持久保存在数据库中。
...
纠正已保存的信息时，明确写出“更正”及人物、原内容和新内容。系统会保留旧断言及其来源，并让新断言替代它。普通新值与旧值冲突时会先澄清；明确为可并存的补充时会保留旧值。当前每次录入最多 20 条输入、合计 24000 字符；每轮最多整理 16 个主体和 32 条断言，超出或无法确定的内容会显示为未处理。
