...
**操作密度** — Profile、NapCat 和消息操作沿用现有页面状态与信息顺序。实体表面保持可读对比；窄屏改为单栏时，主要触控目标保持足够尺寸。装饰背景不承载信息，也不遮挡输入、焦点或错误提示。
...
Kaguya 按模块职责组织视图：注意力展示唤醒状态、观察或延后、触发信号、Focus、未读数量与注册水位；表达展示情境、风格与验证结果；记忆展示实际文档和索引元数据；调度与回合展示历史及来源。每个第一方模块在 Manifest 中声明检查视图与机制说明，由检查 API 从已有事实账本生成可读字段，不另外维护一份运行事件库。
