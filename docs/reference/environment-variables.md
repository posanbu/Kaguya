...
入站列表只决定外部消息是否进入 Runtime；拒绝时不会创建 turn。出站列表用于目标授权和最终投递检查，拒绝时不调用 transport，记录 `destination-not-allowed` 安全错误码。两套权限互不继承，可配置只接收或只发送；自然语言跨会话由宿主验证本轮 Planner 目标，管理端发起的跨会话请求仍需确认。
