/**
 * 功能概述：独立的注意力设计预览入口，以演示状态承载生产 ModuleSurface，便于用户实时引导。
 * 外壳保持紧凑的页标题，避免预览面包屑和说明层级挤占实际工作区。
 * 主要职责：Preview 加载正式 Manifest 并切换正常、空库、错误与加载场景及浅深主题；PreviewTrace 用生产只读 API 展示来源及引用。
 * 代码库关系：只由 preview-arousal.html 加载，不进入正式 main.tsx；前端源码修改通过 Vite HMR 更新；正式 surface 未就位时回退 ModuleRuntimeSection。
