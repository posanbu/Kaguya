/**
 * 功能概述：启动 #189 的本地热更新预览，用真实只读 Inspection API 读取隔离的演示账本。
 * 主要职责：main 准备两个内存 PGlite 库，Vite 承载正式 React 组件并转发 GET 至 Fastify 注入接口；信号退出关闭全部资源。
 * 代码库关系：根 preview:association 命令调用本入口，association-fixture 提供数据，前端 preview-association.html 提供状态切换。
 * 输入输出与副作用：只监听 127.0.0.1:5189；不读取用户配置或生产库、不调用模型；preview-* 标记仅用于此独立进程的演示状态。
 */
...
async function main() {
  ...
  for (const [index, database] of databases.entries()) {
    ...
    if (index === 0) await seedAssociationPreview(database);
    const app = Fastify();
    registerInspectionRoutes(
      app,
      createInspectionService({
  ...
  const vite = await createServer({
    ...
    plugins: [
      {
        name: "association-preview",
        configureServer(server) {
          server.middlewares.use(async (req, res, next) => {
            if (req.url === "/__preview/module") {
              res.setHeader("Content-Type", "application/json");
              res.end(JSON.stringify(associationPreviewModule));
              return;
            }
  ...
  await vite.listen();
  console.log(
    "记忆联想实时预览：http://localhost:5189/preview-association.html",
  );
  const close = async () => {
    await vite.close();
